# 🦷 Two-Stage Cascade Framework for Dental Caries Detection

**Project Goal:** Developing a robust diagnostic pipeline that decouples lesion localization from fine-grained texture verification.

---

## 🛠️ Engineering Overview

This project represents a transition from a failing baseline to a high-performance hybrid system. As an aspiring **Support & Systems Engineer**, my focus was on building a stable data pipeline and a reliable decision-making logic for medical diagnostics.

### 📂 Project Structure

#### [M2: Image-Level Baseline](./M2_Baseline_Classifier/)

- **Approach:** Binary classification using ResNet50 on 342 Panoramic (OPG) ROIs.
- **Outcome:** 51.92% Accuracy.
- **Engineering Insight:** Identifed that the model suffered from "Resolution Bottleneck"—it couldn't see incipient caries without localized focus.

#### [M3: Hybrid Cascade System](./M3_Cascade_System/) (Original Contribution)

- **Stage 1 (The Spotter):** YOLOv8 Medium configured for high recall (0.15 threshold) to flag all suspicious regions.
- **Stage 2 (The Expert):** ResNet50 trained on a custom-engineered dataset of 18,000 tooth patches (512x512) with dynamic padding.
- **Hybrid Logic:** A custom inference engine `D(x)` that escalates ambiguous YOLO detections to the ResNet verifier for pixel-level texture analysis.
- **Outcome:** 93.8% mAP@50.

---

## 📈 Key Technical Contributions

- **Data Engineering:** Built an automated pipeline to expand the dataset from 678 OPGs into 18,000+ annotated patches, mitigating the severe overfitting seen in the baseline.
- **Troubleshooting:** Diagnosed False Positives generated by metallic artifacts (fillings/crowns) and implemented a second-stage verification to filter them out.
- **Reproducibility:** All experiments use fixed random seeds (42) and standardized ImageNet normalization to ensure consistent results across different clinical environments.

---

## 📄 Documentation

Detailed academic reports following the IEEE format are available in the [Documentation folder](./Documentation):

1. **[Literature Review](./Documentation/M1_Literature_Review.pdf):** Analysis of 10+ academic papers on dental AI.
2. **[Final Research Paper](./Documentation/M4_Final_Paper.pdf):** Full 8-page aggregate report of methodology and results.

---

## 🔧 Setup

```bash
# Clone the repository
git clone [https://github.com/yourusername/Dental-Caries-Cascade.git](https://github.com/yourusername/Dental-Caries-Cascade.git)

# Install requirements
pip install -r requirements.txt
```
